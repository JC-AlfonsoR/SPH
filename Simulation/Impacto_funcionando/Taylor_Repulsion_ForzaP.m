...
T_dx = T_dy/2;   %     
k = 5.0;   %    Constante para expandir radio de soporte
...
%
s_x = T_dx/20;             % Separacion en x entre proyectil y objetivo
B_width = 0.002;    % Ancho del proyectil
...
B_dx = 0.7e-4;            % Separacion entre particulas del proyectil
B_dy = 2e-4;
...
B_E = B_ss^2*T_rho;     % Modulo de Young
...
for ti = 1:steps
    ...
    V1(1:T_np) = V1(1:T_np) + dV1(1:T_np)*dt*1e3;
    V2(1:T_np) = V2(1:T_np) + dV2(1:T_np)*dt*1e6;
    ...
    %%%Avanzar la velocidad
    V1(T_np+1:N_part) = V1(T_np+1:N_part) + dV1(T_np+1:N_part)*dt*1e-12;
    V2(T_np+1:N_part) = V2(T_np+1:N_part) + dV2(T_np+1:N_part)*dt*100000000;
    ...
    Particles(T_np+1:N_part,1) = Particles(T_np+1:N_part,1) + ...
        V1(T_np+1:N_part)*dt;
    Particles(T_np+1:N_part,2) = Particles(T_np+1:N_part,2) + ...
        V2(T_np+1:N_part)*dt;
    ...
    if mod(ti,1)==0
        ...
        subplot(2,1,2)
        scatter(Particles(:,1),Particles(:,2),10,V2,'filled')
        %plot(V2,'Marker','o')
        title('Velocidad 2')
        %caxis([-1,1])
        colorbar()
