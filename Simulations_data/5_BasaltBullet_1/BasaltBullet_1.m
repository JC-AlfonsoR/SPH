%% Jc_tensile
%{
Con este script se busca simular el impacto de un proyectil duadrado de
basalto sobre un objetivo del mismo material.
%}
...
%}
dy = 0.000384848;   %   [1x1] separacion entre particulas para el objetivo
dx = dy;
d_bullet = dy;    %   [#] separacion entre particulas para el proyectil
s_bullet = 2*dy;    %   [#] Separacion incial entre proyectil y objetivo
l_bullet = 10*dy;    %   [#] Tamaño de la bala cuadrada
...
ro2 = 2750;         %   [#] Densidad aluminio ~ creo que es la densidad 
                    %        del proyectil
platestart = -2*bar_width; %[#] Dominio 2D debe empezar en esta posiciï¿½n 
plateend = 2*bar_width;    %[#] Dominio 2D debe terminar en esta posiciï¿½n
...
bullet_xi = -bar_width/2 - s_bullet - l_bullet;
bullet_yi = max(max(ybar))/2 - l_bullet/2;
x_b = (bullet_xi) : (d_bullet) : (bullet_xi + l_bullet);
y_b = (bullet_yi) : (d_bullet) : (bullet_yi + l_bullet);
...
% relaciï¿½n al nï¿½mero de cada partï¿½cula.
...
% vez que pasa por una matriz de Flaws, le imprime el nï¿½mero 
...
D = zeros(numpart,1);           %[numpart x 1] Daï¿½o de cada partï¿½cula [0 1]
cs = ss*ones(numpart,1);        %[numpart x 1] ~Velocidad del sonido

%% Velocidades
V1 = zeros(numpart,1);          %[numpart x 1] Velocidad 1 -> 0's
V2 = ones(numpart,1);           %[numpart x 1] Velocidad 2 -> 1ï¿½s
...
% los recorridos, Vdistr se convierte a columna y se guarda en V2. Asï¿½,
% Vdistr se repite 21 veces dentro de V2. Creo que el #67 se debe a la
% forma en la que esta creada la malla, indicando que la distribuciï¿½n de
% velocidad se realiza sobre 67 particulas en fila.
V1(numpart-n_bullet+1: numpart) = 300;
            % Particulas del proyectil viajan a 300 m/s

Velplot = zeros(size(X));       % [Matriz] Matriz para graficar. Tiene el 
                                %   mismo tamaï¿½o que la matriz de malla X.
for i=1:numpart
   Velplot(i) = V2(i);
end
...
% Grafica Velplot como una imï¿½gen. En x se presentan 21 puntos y en y se
% presentan 67 puntos. La velocidad no cambia con x.
scatter(coorbar(:,1),coorbar(:,2),5,V1,'filled');
...
%%
    ...
    %   que contiene los indices de las n particulas vecinas a la partï¿½cula
    %   de identidad i.
    %   Dist        cell {npart x 1} 
    ...
    %   particulas vecinas de la partï¿½cula i.
    ...
    %   kern    cell {npart x 1}
    %   cellfun permite aplicar una funciï¿½n a cada elemento del cell. En
    %   este caso multiplica todas las matrices por 0. Se hace para inciar
    %   el arreglo en 0's.
    
    dkernx = kern;  %   cell {npart x 1} 
    dkerny = kern;  %   Derivadas del kern iniciadas en 0ï¿½s
...
%{ 
    Para la partï¿½cula i, se calcula el kernel evaluado en sus n partï¿½culas
    vecinas.
    Para la partï¿½cula i, se calculan las derivadas x & y del kernel
    evaluados en las particulas vecinas.  
%}
        %%
        % Ecuaciï¿½n de estado Mie Gruniensen
        P(i) = EOSmie(eint(i), r0, C, S, Rho(i), gamma);
%{
    Para la partï¿½cula i, se calcula la presiï¿½n hidrostï¿½tica en funciï¿½n de
    las demas variables de Estado. El calculo se hace usando la Ecuaciï¿½n de
    Mie-Grniensen. Las variables de estado consisten en enrgï¿½a interna,
...
%}
        ...
        [dv1dx1(i), dv1dx2(i), dv2dx1(i), dv2dx2(i)] = ...
            ...
            Nearpart{i,1}, i);
%{
    Para la partï¿½cula i, se calculan las derivadas espaciales en
    direcciones 1 y 2 para las componentes de velocidad V1 y V2. El cï¿½lculo
    se hace con aproximaciï¿½n por partï¿½culas vecinas.
%}
        %%
        % Deformacion unitaria
        [eps11(i), eps12(i), eps21(i), eps22(i)] = ...
            deform(dv1dx1(i), dv1dx2(i), dv2dx1(i), dv2dx2(i));
%   Para la partï¿½cula i, calcula lasc componentes del tensor dedeformaciï¿½n
...
%  Para la partï¿½cula i, calcula las componentes del tensor de esfuerzos              
...
%{
    ...
    los esfuerzos para la partï¿½cula i se calculan con un incremento
...
%}
       ...
       %Calculo del daï¿½o
        ...
        % Alivio de Esfuerzos generado por daï¿½o
        %%
        % Ecuaciï¿½n de Continuidad
...
%% Referencias
%{
[1] D.Luna & A. Gonzalez, Estudio computacional de la fragmentaciï¿½n de
materiales fragiles con el mï¿½todo de particulas suavizadas (SPH), Uniandes,
2015.
